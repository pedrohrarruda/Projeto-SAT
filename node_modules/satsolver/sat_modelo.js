...
function nextAssignment(currentAssignment) {
 var quantos = 0
 formula.variables[quantos] = true
  let resuts = []
...
function doSolve(clauses, assignment) {
  ...
  while ((!isSat) && (!resultado) && (!acabou)) {
    ...
    assignment = nextAssignment(formula.variables)
    contador++
  }
...
function readFormula(filename) {
  ...
  const fs = require('fs')
  var text = fs.readFileSync('C:/Users/PMRecife/Desktop/sat-master/'+filename+'.cnf', 'utf8').split(/[\r\n]+/)   
  ...
  for(a = 0; a<clauses.length;a++){
    for(b = 0 ;b<clauses[a].length;b++){
      if(clauses[a][b] > array.length){
        for(c = 0;c<Math.abs(clauses[a][b]);c++){
        array.push(false)
      }
      }
    }
